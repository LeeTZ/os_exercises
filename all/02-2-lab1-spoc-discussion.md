...
```
 ...
 ```
- 建立一个中断描述符表（IDT），当一个中断产生的时候会有对应的终端号，比如时钟中断会有对应的中断号。用这个 终端号作为索引，设置中断描述符表对应索引的一项，指向中断服务例程的地址。这个地址中包括段选择子和偏移，段描述符存在 于全局描述表中，根据段选择子来查询段描述符。根据段描述符确定中断服务例程的基址，加上偏移量就形成了中断服务例程的地址 最后利用lidt指令，告知CPU完成中断描述符表的加载，和IDT的位置。以上完成了8259中断控制器的初始化，和IDT的初始化。然后进行外设的 初始化，对8253时钟的初始化，要使中断使能，使它能够每100个tick产生一个中断。 
